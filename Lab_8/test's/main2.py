...
class QueryDialog:
    def __init__(self, parent, db_path, query_type):
        self.dialog = tk.Toplevel(parent)
        self.db_path = db_path
        self.query_type = query_type
        self.result = None

        self.dialog.title(f"Выполнение запроса: {query_type}")
        self.dialog.geometry("400x500")

        self.create_widgets()

    def create_widgets(self):
        # Создаем поля для ввода параметров
        param_frame = ttk.Frame(self.dialog)
        param_frame.pack(fill='x', padx=5, pady=5)

        self.entries = []
        for i in range(6):
            ttk.Label(param_frame, text=f"Параметр {i+1}:").pack(pady=5)
            entry = ttk.Entry(param_frame)
            entry.pack(pady=5)
            self.entries.append(entry)

        # Кнопка выполнения
        ttk.Button(self.dialog, text="Выполнить", command=self.execute_query).pack(pady=10)
    ...
    def execute_query(self):
        try:
            ...
            param_values = [entry.get() for entry in self.entries]

            # Выполняем запрос с параметрами
            cursor.execute(f"EXEC [{self.query_type}] ?, ?, ?, ?, ?, ?", param_values)
...
class MedicalDBApp:
    def __init__(self, root):
        ...
        # Загружаем запросы из базы данных
        self.load_queries()
    ...
    def load_queries(self):
        try:
            ...
            # Измененный SQL-запрос для получения нужных типов
            cursor.execute("""
                SELECT Name, Type
                FROM Копия_MSysObjects
                WHERE ([Type] = 5)
            """)

            self.queries = {row.Name: row.Type for row in cursor.fetchall()}
            conn.close()

        except Exception as e:
            messagebox.showerror("Ошибка", f"Ошибка загрузки запросов:\n{str(e)}")
            self.queries = {}
    ...
    def create_widgets(self):
        ...
        # Добавляем новую панель для запросов
        query_frame = ttk.LabelFrame(self.root, text="Запросы")
        query_frame.pack(fill='x', padx=5, pady=5)

        # Выпадающий список запросов
        self.query_var = tk.StringVar()
        self.query_combo = ttk.Combobox(query_frame, textvariable=self.query_var,
                                        values=list(self.queries.keys()))
        self.query_combo.pack(side='left', padx=5)

        ttk.Button(query_frame, text="Выполнить запрос",
                   command=self.execute_selected_query).pack(side='left', padx=5)
        ...
        ttk.Button(frame_top, text="Системные таблицы", command=self.show_sys_objects).pack(side='left', padx=5)

    def show_sys_objects(self):
        try:
            conn = self.get_connection()
            cursor = conn.cursor()

            cursor.execute("SELECT * FROM Копия_MSysObjects WHERE Name NOT LIKE 'MSys%'")

            # Очищаем таблицу перед показом
            ...
            messagebox.showerror("Ошибка", f"Ошибка при отображении системных объектов:\n{str(e)}")
    ...
    def execute_selected_query(self):
        ...
        if not query_name:
            messagebox.showwarning("Предупреждение", "Выберите запрос")
            return
        ...
        try:
            ...
            if self.queries[query_name] == 5:  # Процедура
                # Создаем диалоговое окно для ввода параметров
                dialog = QueryDialog(self.root, self.db_path, query_name)
                self.root.wait_window(dialog.dialog)
            else:
                # Выполняем как обычный SELECT-запрос
